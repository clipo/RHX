...
if sys.platform == "win32":
   debugfilename = "c:/Users/Archy/Dropbox/Rehydroxylation/Logger/Autosampler/logs/rhx-DataReadWrite-" + datestring + "_debug.log"
elif sys.platform == "darwin":
   debugfilename = "/Users/Clipo/Dropbox/Rehydroxylation/Logger/Autosampler/logs/rhx-startRHX" + datestring + "_debug.log"
else:
...
if sys.platform == "darwin":
   SQLFILENAME = "/Users/Clipo/Dropbox/Rehydroxylation/RHX-New.sql"
else:
   SQLFILENAME = "c:/Users/Archy/Dropbox/Rehydroxylation/RHX-New.sql"

XYDBFILENAME=""
if sys.platform == "darwin":
   XYDBFILENAME = "/Users/Clipo/Dropbox/Rehydroxylation/RHX.sqlite"
else:
...
path = '/Users/Clipo/Dropbox/Rehydroxylation/Logger/Autosampler/libsqlitefunctions.so'
...
if sys.platform=="win32":
   ...
   path = '/Users/Archy/Dropbox/Rehydroxylation/Logger/Autosampler/libsqlitefunctions.so'
...
def initializeDatabase(dirname, filename):
   ...
   try:
      ...
      if sys.platform=="win32":
         ...
         dbfilename="/Users/Archy/Dropbox/Rehydroxylation/RHX.sql"
      else:
...
def getRunInfo(runID):
   ...
   try:
      ...
      c = conn.cursor()
      logger.debug(
         "select v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_endOfFiring,i_durationOfFiring from tblRun where i_runID = %d" % (
         runID))
      c.execute(
         'select v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_endOfFiring,i_durationOfFiring from tblRun where i_runID=%s' % runID)
      row = c.fetchone()
      return (status, row)
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s" % msg)
...
def updateRunPostFire(runID, setInitials, status, d_dateTimeFiring,
   ...
   logger.debug("updateRunPostFire (   %d,%s,%s,%s,%d,%d,%d,%d,%s,%d )" %
                (runID, setInitials, status, d_dateTimeFiring, durationOfFiring, temperatureOfFiring,
   ...
   # create a cursor to the database
      ...
      # Insert initial data for the run
      logger.debug(
         'update tblRun SET i_loggingInterval=%d,v_status=%s,d_dateTimeFiring=%s,i_durationOfFiring=%d,i_temperatureOfFiring=%d,i_postMeasurementTimeInterval=%d,i_durationOfPostMeasurements=%d,d_endOfFiring=%s WHERE i_runID=%d' % (
         t))
      c.execute(
         'update tblRun SET i_loggingInterval=?,v_status=?,d_dateTimeFiring=?,i_durationOfFiring=?,i_temperatureOfFiring=?,i_postMeasurementTimeInterval=?,i_durationOfPostMeasurements=?,d_endOfFiring=? WHERE i_runID=?'
         , t)
      # Save (commit) the changes
      conn.commit()
      # Now get the id for the run so we can update with other info as we ask...
...
def getXYCoordinatesForBalance(point):
   inOrOut = ""
   if point == "outside":
      inOrOut = "BALANCE_OUTSIDE"
   else:
...
def updateXYCoordinatesForBalance(position, x, y):
