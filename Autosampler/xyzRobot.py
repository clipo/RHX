...
SLEEPTIME = 15       ## length of sleep before starting to weigh the sample on the balance
...
### INSIDE BALANCE POSITION (11/17):  X:  32000 Y: 162500 (Rollers: X:3211 Y:16679)
...
def PositionChanged(e):
    source = e.device
    logger.debug("AdvancedServo %i: Motor %i Position: %f - Velocity: %f - Current: %f" % (
        source.getSerialNum(), e.index, e.position, velocityList[e.index], currentList[e.index]))
...
def getTemperature():
    try:
        ...
        logger.critical("Error:  %i: %s" % (e.code, e.details))
        return False
    sensor = float((value * 0.222222222222) - 61.1111111111 + 2.5) ## 2.5 added to match madgetech
    return sensor
...
def getHumidity():
    try:
        ...
        logger.critical("Error:  %i: %s" % (e.code, e.details))
        return False
    sensor = float((value * 0.1906) - 40.20000 - 5.92) ## 5.22 added to match madgetech
    return sensor
...
def refineInsideBalancePosition():
    ...
    xdiff=xpos-xCrucible  ## larger means to the left - so bump down
    ydiff=ypos-yCrucible ## larger means forward so bump right
    logger.debug("now check when xdiff is less than zero. xdiff: %d " % xdiff)
    if xdiff<0:
       while (abs(xpos-xCrucible)>0):
          bumpXMotorUp(bump)
          xpos,ypos=myLocation.getPositions()
...
def goToInsideBalanceFromOutside():
    ...
    logger.debug("Now moving Y position to inside balance position:  %d", yBalance)
    ...
    logger.debug("now moving x motor to inside balance position: %d", xBalance)
    try:
        xstepper.setEngaged(0, True)
        xstepper.setTargetPosition(0, xBalance)
        while xstepper.getStopped(0) != STOPPED:
...
def weighAllCrucibles(initials, numberOfSamples, loggingInterval,
                      ...
                      SAMPLE_POSITION, MCOUNT, CURRENTSTEP, STATUS,
    ...
    logger.debug(
        "weightAllCrucibles: %s, %d, %d, %d, %d" % (
        initials, numberOfSamples, loggingInterval, duration, startPosition))
    position = int(startPosition)
    #HomePosition()
    listOfValues = ()
    LOGGERINTERVAL.set(loggingInterval)
    DURATION.set(duration)
    SAMPLE_POSITION.set(position)
    ...
    Label(robotStatus, textvariable=SAMPLE_POSITION).grid(row=1, column=1, sticky=W)
    ...
    if position is "" or position is 0 or position is None:  ## position must start at 1
        position = 1
    ...
    while position < (numberOfSamples + 1):  # positions go from 1 to 25 (may need to fix one day)
        ...
        averageLight = 0.0
        robotStatus.update()
        ## sleep for SLEEPTIME seconds -- dont record the first N minutes of data - let the balance recover
        sleep(SLEEPTIME)
            ...
            if weight > 0.0:
                ...
                if humidity > 0:
                    ...
                    if humidityArray.size > 1:
                        ...
                        stdevHumidity = 0.0
                count += 1
                logger.debug("Count: %d the average weight of crucible # %i is: %f with stdev of: %f" % (
                    count, position, averageWeight, stdevWeight))
                ...
                statustext = "Weight recorded for crucible #%d with %d counts %f weight %f stddev" % (
                    position, count, averageWeight, stdevWeight)
                STATUS.set(statustext)
...
def weighAllSamplesPreFire(runID, duration, loggingInterval,
                           ...
                           robotStatus, SAMPLE_POSITION, MCOUNT, CURRENTSTEP,
    ...
    if position == "":
        position = 1
    logger.debug(
        "weighAllSamplesPreFire( %d,%d,%d,%d,%d )" % (runID, duration, loggingInterval, numberOfSamples, startPosition))
    ...
    while (position < (numberOfSamples + 1)):
        ...
        goHome()
        SAMPLE_POSITION.set(HOME)
        crucibleWeight = float(DataReadWrite.getChamberCrucibleWeight(position))
        if crucibleWeight is False:
            alertWindow("getCrucibleWeight returned an error.")
            return False

        sampleID, t_sampleName, t_assemblageName = int(DataReadWrite.getSampleID(runID, position))
        startTime = datetime.today()
        durationOfLogging = int(duration)
        endPoint = timedelta(minutes=durationOfLogging)
        endTime = startTime + endPoint
        ...
        while datetime.today() < endTime:
            ...
            weight = float(measurement) - float(crucibleWeight)
            if weight > 0.0:
                a = append(a, float(weight))
                averageWeight = a.mean()
                stdevWeight = a.std()
                count += 1
                logger.debug("Count: %d the average weight of sample # %d is %f with stdev of %f" % (
                    count, position, averageWeight, stdevWeight))
                ## now update crucible position record
                now = datetime.today()
                today = now.strftime("%m-%d-%y %H:%M:%S")
                ...
                if (count > COUNTS_FOR_STATS):
                    (mean, stdev, variance, tempMean, tempStdev, humidityMean,
                     humidityStdev) = DataReadWrite.getStatsForPrefireWeightOfSample(runID, sampleID, count)
                    MEAN.set(mean)
                    STDEV.set(stdev)
                    VARIANCE.set(variance)
                    logger.debug(
                        "Mean: %d  Stdev: %d  Variance: %d TempMean: %d  TempStDev: %d HumidityMean: %d HumidityStdDev: %d" % (
                            mean, stdev, variance, tempMean, tempStdev, humidityMean, humidityStdev))
                    value = DataReadWrite.updateSamplePreFire(runID, sampleID, position, mean, stdev, tempMean,
                                                              tempStdev,
                                                              humidityMean, humidityStdev)
                sleep(loggingInterval)
        ...
        SAMPLE_POSITION.set(int(position))
...
def weighAllSamplesPostFire(runID, duration,
                            ...
                            SAMPLE_POSITION, MCOUNT, CURRENTSTEP, STATUS, DURATION,
                            LOGGERINTERVAL, RUNID, NUMBEROFSAMPLES, TIMEREMAINING, TIMEELAPSEDMIN, REPS, CYCLE):
    logging.debug("weighAllSamplesPostFire( %d,%d,%d,%d,%d,%s,%f,%f,%d)" % (runID, duration,
                                                                            loggingInterval, numberOfSamples,
                                                                            startPosition, endOfFiring, tempCorrection,
                                                                            rhCorrection, repetition))
    # Find elapsed time
    #first put robot back to zero
    position = int(startPosition)
    ...
    while position < maxPosition:
        ...
        sampleID, t_sampleName, t_AssemblageName = DataReadWrite.getSampleID(1, position)
        startTime = datetime.today()
        durationOfLogging = int(duration)
        endPoint = timedelta(minutes=durationOfLogging)
        ...
        count = 0
        robotStatus.update()
        ## sleep for NNN seconds -- dont record the first two minutes of data - let the balance recover
        sleep(SLEEPTIME)
        while datetime.today() < endTime:
            robotStatus.update()
            timeLeft = endTime - datetime.today()
            ...
            if weight > 0.0:
                a = append(a, float(weight))
                averageWeight = float(a.mean())
                stdevWeight = float(a.std())
                count += 1
                logger.debug("Count: %i the average weight of sample # %i is %f with stdev of %f" % (
                    count, position, averageWeight, stdevWeight))
                ## now update crucible position record
                now = datetime.today()
                today = now.strftime("%m-%d-%y %H:%M:%S")
                ...
                robotStatus.update()
                ## timeElapsedQuarterPower=float(pow(timeElapsedMin,0.25))

                value = DataReadWrite.insertPostFireMeasurement(runID, sampleID, position, weight, status, temperature,
                                                                humidity,
                                                                endOfFiring, crucibleWeight, standard_weight, now,
                                                                total_count, repetition, repetition_count, count)
                if (value is False):
                ...
                if (count > COUNTS_FOR_STATS):
                    (mean, stdev, variance, tempMean, tempStdev, humidityMean,
                     humidityStdev) = DataReadWrite.getStatsForPostFireWeightOfSample(runID, sampleID, count)
                    if mean is None:
                        mean = 0.0
                    if variance is None:
                        variance = 0.0
                    ...
                    logger.debug(
                        "Mean: %d  Stdev: %d  Variance: %d TempMean: %d  TempStDev: %d HumidityMean: %d HumidityStdDev: %d" % (
                            mean, stdev, variance, tempMean, tempStdev, humidityMean, humidityStdev))
                    value = DataReadWrite.updateSamplePostFire(runID, position, tempMean, tempStdev, humidityMean,
                                                               humidityStdev, count, repetition, timeElapsedMin)
                sleep(loggingInterval)
...
def bumpXMotorUp(bump):
    ...
    xstepper.setEngaged(0, True)
    currentMotorPosition = int(xstepper.getCurrentPosition(0))
...
def bumpYMotorLeft(bump):
    ...
    currentPosition = ystepper.getCurrentPosition(0)
    if currentPosition < 0:
        currentPosition = 0

    move = currentPosition - int(bump)
    if move < 0:
        move = 0
...
def bumpXMotorDown(bump):
    ...
    currentPosition = xstepper.getCurrentPosition(0)
    if currentPosition < 0:
        currentPosition = 0
    moveVal = currentPosition - int(bump)
    if moveVal < 0:
        moveVal = 0
...
def resetXYMotorsToZeroPosition():
    ...
    try:
        ...
        while (getAbsoluteXPosition() - 1 > MINIMUM_X_VALUE):
            ...
            logger.debug(
                "Now X Motor at X: %d Y: %d" % (xstepper.getCurrentPosition(0), ystepper.getCurrentPosition(0)))
    ...
    try:
        ...
        while (getAbsoluteYPosition() - 1 > MINIMUM_Y_VALUE):
            ...
            logger.debug(
                "Now Y Motor at X: %d Y: %d" % (xstepper.getCurrentPosition(0), ystepper.getCurrentPosition(0)))
            logger.debug("Now Abs Position at X: %d Y: %d " % (getAbsoluteXPosition(), getAbsoluteYPosition()))
            move = ystepper.getCurrentPosition(0) - bump
            while ystepper.getStopped(0) != STOPPED:
                pass
...
def initializeRobot():
    ...
    try:
        ...
        logger.critical("Runtime Exception -- cannot create stepper objects: %s" % e.details)
        logger.critical("Exiting....")
        return False
    ...
    try:
        ...
        logger.critical("Phidget Exception: cannot advancedServo.openPhidget %i: %s" % (e.code, e.details))
        logger.critical("Exiting....")
        sys.exit(1)
    ...
    try:
        ...
        logger.critical("Phidget Exception: advancedServo.waitForAttache(10000) %i: %s" % (e.code, e.details))
        try:
            advancedServo.closePhidget()
        except PhidgetException as e:
            logger.critical("Phidget Exception : advancedServo.closePhidget %i: %s" % (e.code, e.details))
            logger.critical("Exiting....")
            return False
        logger.critical("Exiting....")
        sys.exit(1)
    ...
    if XMOTOR == 0:
        try:
            ...
            logger.critical("Phidget Exception: cannot open xmstepper.openPhidget(XMOTOR) %i: %s" % (e.code, e.details))
            logger.critical("Exiting....")
            sys.exit(1)
    else:
        try:
    ...
    logger.debug("Waiting for xstepper motors to attach....")
    try:
        xstepper.waitForAttach(1000)
    except PhidgetException as e:
        logger.critical("Phidget Exception MOTOR WOULDNT ATTACH xstepper.waitForAttach(1000) %i: %s" % (e.code,
                                                                                                        e.details))
