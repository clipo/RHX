...
## logging setup
logger=logging.getLogger("startRHX.startRHX")
...
def update_windows():
   ...
   XDISTANCE.set(xyzRobot.getXDistance())
   YDISTANCE.set(xyzRobot.getYDistance())
...
def go_initialize():
   ...
   logger.debug( "Done!  The next script to run will be preFire.py ")
...
def go_setup():
   ...
   if runID is False:
      ...
      alert.deiconify()
      Message(alert,text="There has been a problem. Cannot insert initial Run", bg='red', fg='ivory',
         relief=GROOVE)
      exit(1)
...
def go_setupPart2():
   ...
   CRUCIBLEWEIGHT.set(0.0)
   SAMPLEWEIGHT.set(0.0)
   POSTFIREWEIGHT.set(0.0)
   INITIALWEIGHT.set(0.0)

   Label(setupCrucibles, text="Mean").grid(row=1, column=1, sticky=W)
   Label(setupCrucibles, text="StdDEv").grid(row=1, column=2, sticky=W)
   Label(setupCrucibles, text="Current Sample").grid(row=1, column=0, sticky=W)
   Entry(setupCrucibles, textvariable=CURRENTSAMPLE).grid(row=1, column=1, sticky=W)
   Label(setupCrucibles, text="Initial Weight of Sherd (pre-firing)").grid(row=2, column=0, sticky=W)
   Entry(setupCrucibles, textvariable=INITIALWEIGHT).grid(row=2, column=1, sticky=W)
   Entry(setupCrucibles, textvariable=INITIALWEIGHTSTDDEV).grid(row=2, column=2, sticky=W)
   ctext="Crucible #"+ str(count)+ " Weight (g)"
   ...
   ENTERBUTTON = Button(setupCrucibles, text="Submit Data for Sample/Crucible", command=insertCrucibleAndSample).grid\
      (row=6, column=0, sticky=W, padx=2, pady=2)
   QUITBUTTON = Button(setupCrucibles, text="End (No more samples)", command=quit_setup).grid(row=6, column=1,
      sticky=W, padx=2, pady=2)
   update_windows()

def insertCrucibleAndSample():
   setupCrucibles.withdraw()
   runID = RUNID.get()
   now = datetime.today()
   today = now.strftime("%m-%d-%y %H:%M:%S")
   ...
   if (value==False):
      ...
      exit(1)
   DataReadWrite.updateCrucible(position,CRUCIBLEWEIGHT.get(),CRUCIBLEWEIGHTSTDDEV.get(),RHTEMP2000TEMP.get(),0.0,RHTEMP2000HUMIDITY.get(),
   ...
   sampleID = DataReadWrite.insertNewSample(runID,position,setName,now,setLocation,preOrPost,0,
      setTemperature,setHumidity,status,0,0,meanWeight,stdevWeight)
...
def go_preFire():
   ...
   now = datetime.today()
   today = now.strftime("%m-%d-%y %H:%M:%S")
   value=DataReadWrite.updateRunPreFire(runID,setInitials,setName,today,setLocation,preOrPost,intervalsec,setTemperature,setHumidity,status,duration,numberOfSamples,repetitions,startPosition)
   if value is False:
      logger.error("There has been an error since updateRunPreFire returned FALSE")
      communication.sendEmail ("RHX Error","An error has occurred with updateRunPreFire!")
      alert.deiconify()
...
def getSampleFromBalance():
   ...
   if (sampleNum<1):
      ...
      closeBalanceDoor()
      val=xyzRobot.goToSamplePosition(sampleNum)
...
def alertWindow(message):
   ...
   alert.deiconify()
   Message(alert,text=message, bg='red', fg='ivory', relief=GROOVE)
...
def findHome():
   xyzRobot.resetMotorsToZeroPosition()
   update_windows()
   return True;

def setAbsZeroXY():
   value=xyzRobot.setAbsZeroXY(xyzRobot.getAbsoluteXPosition(), xyzRobot.getAbsoluteYPosition())
   return True;
...
def setXYForSampleLocation():
   ...
   xD,yD=xyzRobot.getCurrentXYDistance()
   value=DataReadWrite.updateDistanceXYForPosition(sampleLocation,xD,yD)
...
def setXYForBalance(point):
   originalX=0
   originalY=0
   (originalX, originalY) = DataReadWrite.getXYForBalance(point)
   x=int(xyzRobot.getXPosition())
   y=int(xyzRobot.getYPosition())
   diffX=originalX - x
   diffY=originalY - y

   value = DataReadWrite.updateXYForBalance(point,x,y)
   return value
...
def checkEachSamplePosition():
   ...
   while sampleLocation < 6:
      ...
      value=xyzRobot.goToSamplePosition(sampleLocation)
...
BDL1 =Label(root, text="Balance Door")
BDL1.grid(row=10, column=0, sticky=W)
BDB1 = Button(root, text="Open", command=openBalanceDoor)
BDB1.grid(row=11, column=0, sticky=W, padx=2, pady=2)
BDB2 = Button(root, text="Close", command=closeBalanceDoor)
BDB2.grid(row=11, column=1, sticky=W, padx=2, pady=2)

#DOOR=Label(root, text="Is Balance Door Open?").grid(row=12,column=0,sticky=W)
#DOOR1=Label(root, textvariable=BALANCEDOOR).grid(row=12,column=1,sticky=W)

##Balance Zeroing
BZL1=Label(root, text="Zero Balance" ).grid(row=13, column=0, sticky=W)
BZB1=Button(root, text="Zero", command=BZero).grid(row=13, column=1, sticky=W, padx=2, pady=2)

BZL2=Label(root, text="Balance Reading (g) ").grid(row=14,column=0,sticky=W)
BZL3=Label(root, textvariable=BALANCEWEIGHT).grid(row=14,column=1,sticky=W)
BZL3=Label(root,textvariable=BALANCESTATUS).grid(row=14,column=2,sticky=W)
...
STANDARDBL=Label(root,text="Standard Balance (g)").grid(row=16,column=2,sticky=W)
STANDARDBE=Label(root,textvariable=STANDARD_BALANCE).grid(row=16,column=3,sticky=W)


#GRIP=Label(root, text="Gripper hold a crucible?").grid(row=16,column=0, sticky=W)
#GRIP2=Label(root, textvariable=CRUCIBLEYESNO).grid(row=16,column=1,sticky=W)
...
SETABSZEROXY=Button(root,text="Set Min X and Y",command=setAbsZeroXY).grid(row=20,column=1)

ABSOLX1=Label(root, text="X Position (roller)").grid(row=21,column=0,sticky=W)
ABSOLX2=Label(root, textvariable=ABSOLUTEXPOSITION).grid(row=21,column=1,sticky=W)
ABSOLY1=Label(root, text="Y Position (roller)").grid(row=21,column=2,sticky=W)
ABSOLY2=Label(root, textvariable=ABSOLUTEYPOSITION).grid(row=21,column=3,sticky=W)

Label(root, text="X Distance").grid(row=22,column=0,sticky=W)
Label(root, textvariable=XDISTANCE).grid(row=22,column=1,sticky=W)
Label(root, text="Y Distance").grid(row=22,column=2,sticky=W)
Label(root, textvariable=YDISTANCE).grid(row=22,column=3,sticky=W)
...
M1B3 = Button(root, text="Open Gripper", command=openGripper).grid(row=24, column=0, padx=2, pady=2)
M1B4 = Button(root, text="Close Gripper", command=closeGripper).grid(row=24, column=1, padx=2, pady=2)
...
BALANCE1=Button(root,text="Go to Balance", command=goToBalance).grid(row=26,column=0,padx=2,pady=2)
M1B5 = Button(root, text="Update Values of Motors", command=update_windows).grid(row=26,column=2,padx=2,pady=2)
M1B6 = Button(root,text="Outside Balance-Move to 0", command=goHome).grid(row=26,column=1,padx=2,pady=2)

M1B7 = Button(root,text="Find Zero and Reset Motors", command=findHome).grid(row=27,column=0,padx=2,pady=2)
M1B8 = Button(root, text="Move to each sample location (1-25)", command=visitEachSampleLocation).grid(row=27,column=1,padx=2,pady=2)
Button(root,text="Check Each Sample Position (5)",command=checkEachSamplePosition).grid(row=27,column=2,padx=2,pady=2)
Label(root, text="RUNID").grid(row=28,column=0,sticky=W)
Label(root, textvariable=RUNID).grid(row=28,column=1,sticky=W)

Button(root,text="Set XY for this position",command=setXYForSampleLocation).grid(row=28,column=2)

Button(root,text="Go to Outside Balance Point", command=goToOutsideBalance).grid(row=29,column=0)
Button(root,text="Set Outside Balance Point",command=setOutsideBalance).grid(row=29,column=1)
Button(root,text="Set Z for Sample Position",command=setZForSampleLocation).grid(row=29,column=2)

Button(root,text="Go to Inside Balance Point", command=goToInsideBalance).grid(row=30,column=0)
Button(root,text="Set Inside Balance Point",command=setInsideBalance).grid(row=30,column=1)
Button(root,text="Set Z for Balance Position",command=setZForBalanceLocation).grid(row=30,column=2)


Label(root,text="Run Number", textvariable=SETRUNID).grid(row=31,column=0)
Button(root,text="Load Run", command=loadRun).grid(row=31,column=1)
Button(root,text="Load Sqlite File", command=reloadFile).grid(row=31,column=2)
Button(root,text="Refine Position", command=refinePosition).grid(row=31,column=3)
##start the other scripts
STBD1 = Button(root, text="Initialize", command=initialize).grid(row=32, column=0, padx=2, pady=2)
STBD2 = Button(root, text="Pre-Fire", command=preFire).grid(row=32, column=1, padx=2, pady=2)
STBD3 = Button(root, text="Post-Fire", command=postFire).grid(row=32, column=2, padx=2, pady=2)
SETUP=Button(root,text="Setup (New)", command=setup).grid(row=32,column=3,padx=2,pady=2)
QUIT=Button(root,text="Quit", command=quit).grid(row=33,column=2,padx=2,pady=2)
