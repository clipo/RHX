...
INSIDE_BALANCE_POSITION=20000
OUTSIDE_BALANCE_POSITION=10000
...
if sys.platform == "win32":
   ...
   if STANDARDBALANCE is True:
      try:
         standard_balance = serial.Serial(port=ComPorts.StandardBalance, baudrate=9600, bytesize=8, parity='N',stopbits=1,timeout=1, xonxoff=0)
      except:
...
def getMaxPreFireCount(runID, position):
   ...
   try:
      ...
      logger.debug(
         ...
         t))
      c.execute(
         "select max(i_count) as max from tblMeasurement where i_runID=? and i_positionNumber=? and i_preOrPost=1", t)
      max = c.fetchone()["max"]
      if (max is None):
         max = 0
      return max
...
def getMaxPostFireCount(runID, position):
   ...
   try:
      ...
      logger.debug(
         ...
         t))
      c.execute(
         "select max(i_count) as max from tblMeasurement where i_runID=? and i_positionNumber=? and i_preOrPost=2", t)
      max = c.fetchone()["max"]
      if (max is None):
         max = 0
      return max
...
def getPrefireAttributes(runID):
   ...
   try:
      ...
      c.execute("select i_repetitions,v_locationCode,i_numberOfSamples,v_operatorName,i_equilibrationDuration,i_preMeasurementTimeInterval, \
         ...
         v_status,i_loggingInterval,i_durationOfPreMeasurements,i_numberOfPreMeasurements from tblRun where i_runID = %d"
                , runID)
...
def getPostfireAttributes(runID):
   ...
   try:
      ...
      c.execute("select i_numberOfSamples,i_durationOfPostMeasurements,i_numberOfPostMeasurements,i_loggingInterval,i_repetitions,f_locationTemperature, \
         f_locationHumidity, i_durationOfFiring,d_datetimeOfFiring, i_rateOfHeating, i_tempOfFiring from tblRun where i_runID = %d"
                , runID)
...
def insertPreFireMeasurement(runID, sampleID, position, weightMeasurement, status,
   ...
   try:
      c = conn.cursor()
      t = (
      runID, sampleID, position, weightMeasurement, status, now, temperature, humidity, standard_weight, total_count,
...
def updateRunInformation(i_numberOfSamples, v_operatorName, v_locationCode,v_description,f_locationTemperature, runID):
    try:
        c = conn.cursor()
        t=(i_numberOfSamples, v_operatorName, v_locationCode,v_description,f_locationTemperature, runID)
        logger.debug("update tblRun set i_numberOfSamples=%d, v_operatorName=%s, v_locationCode=%s,v_description=%s,f_locationTemperature=%s where i_RunID=%d" % (t))
        c.execute('update tblRun set  i_numberOfSamples=?, v_operatorName=?, v_locationCode=?,v_description=?,f_locationTemperature=? where i_RunID=?', t)
        conn.commit()
        return True
    except sqlite.OperationalError, msg:
        logger.error("A SQL error occurred: %s", msg)
...
def updateRunInfoWithFiringInformation(d_dateTimeOfFiring, i_temperatureOfFiring, i_durationOfFiring, runID):
   ...
   try:
      c = conn.cursor()
      t=(d_dateTimeOfFiring, i_durationOfFiring, d_endOfFiring, i_temperatureOfFiring, i_durationOfFiring, runID)
      logger.debug("update tblRun set d_dateTimeOfFiring=%s,i_durationOfFiring=%d, d_endOfFiring=%s, i_temperatureOfFiring=%d, i_durationOfFiring=%d where i_RunID=%d" % (t))
      c.execute('update tblRun set  d_dateTimeOfFiring=?,i_durationOfFiring=?, d_endOfFiring=?, i_temperatureOfFiring=?, i_durationOfFiring=? where i_RunID=?', t)
      conn.commit()
      return True
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s", msg)
...
def getRunInfo(runID):
   ...
   try:
      c = conn.cursor()
      logger.debug(
         "select v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_dateTimeFiring, i_durationOfFiring,i_temperatureOfFiring, v_operatorName from tblRun where i_runID = %d" % (
         runID))
      c.execute(
         'select v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_dateTimeFiring ,i_durationOfFiring,i_temperatureOfFiring, v_operatorName from tblRun where i_runID=%s' % runID)
      data = c.fetchone()
      ...
      return v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_dateTimeFiring ,i_durationOfFiring,i_temperatureOfFiring, v_operatorName
...
def weighAndRecordSamplePreFire(runID, sampleID, positionNumber, duration, loggingInterval, tempCorrection,
                                rhCorrection):
...
## sequence of steps for weighing sample for a period of time. 
def weighAndRecordSamplePostFire(runID, sampleID, positionNumber, duration, loggingInterval, tempCorrection,
                                 rhCorrection):
   logger.debug("Weight and record sample pre fire")
   stopCode = 0
   lengthOfTimeCounter = int((duration * 60) / loggingInterval)
...
def getInitialSherd(position, runID):
   ...
   try:
      ...
      if (data is None):
         return False;
      return double(data["f_sherdWeightInitialAverage"]), double(data["f_sherdWeightInitialStdDev"])
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s", msg)
      return False;
   return True;
...
def getEmptyCrucible(position, runID):
   ...
   try:
      ...
      if (data is None):
         return False;
      return double(data["f_emptyWeightAverage"]), double(data["f_emptyWeightStdDev"]), int(data["i_emptyWeightCount"])
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s", msg)
      return False;
   return True;
...
def get105Crucible(position, runID):
   ...
   try:
      ...
      t = (position, runID)
      logger.debug(
         'select f_105WeightAverage, f_105WeightStdDev, i_105WeightCount from tblCrucible WHERE i_positionNumber=%d AND i_runID=%d' % t)
      c.execute(
         'select f_105WeightAverage, f_105WeightStdDev, i_105WeightCount from tblCrucible WHERE i_positionNumber=? AND i_runID=?'
         , t)
      data = c.fetchone()
      if (data is None):
         return False;
      return double(data["f_105WeightAverage"]), double(data["f_105WeightStdDev"]), int(data["i_105WeightCount"])
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s", msg)
      return False;
   return True;
...
def get550Crucible(position, runID):
   logger.debug("get crucible weight for 105 prefire... ")
   now = datetime.today()
   try:
      c = conn.cursor()
      t = (position, runID)
      logger.debug(
         'select f_550WeightAverage, f_550WeightStdDev, i_550WeightCount from tblCrucible WHERE i_positionNumber=%d AND i_runID=%d' % t)
      c.execute(
         'select f_550WeightAverage, f_550WeightStdDev, i_550WeightCount from tblCrucible WHERE i_positionNumber=? AND i_runID=?'
         , t)
      data = c.fetchone()
      if (data is None):
         return False;
      return double(data["f_550WeightAverage"]), double(data["f_550WeightStdDev"]), int(data["i_550WeightCount"])
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s", msg)
      return False;
   return True;
...
def updateRunPreFire( runID, setInitials, setName, today,
   ...
   try:
      ...
      # Insert initial data for the run
      logger.debug(
         'update tblRun SET v_operatorName=%s,v_description=%s,d_datetime=%s,v_locationCode=%s,i_loggingInterval=%d,f_locationTemperature=%f,f_LocationHumidity=%f,v_status=%s,i_preMeasurementTimeInterval=%d,i_numberOfSamples=%d,i_numberOfPreMeasurements=%d,i_durationOfPreMeasurements=%d WHERE i_runID=%d' % (
         t))

      c.execute(
         'update tblRun SET v_operatorName=?,v_description=?,d_datetime=?,v_locationCode=?,i_loggingInterval=?,f_locationTemperature=?,f_locationHumidity=?,v_status=?,i_preMeasurementTimeInterval=?,i_numberOfSamples=?,i_numberOfPreMeasurements=?,i_durationOfPreMeasurements=? WHERE i_runID=?'
         , t)
      # Save (commit) the changes
      conn.commit()
      return runID
...
def insertInitialSherd(i_runID,i_crucibleID,f_sampleWeightInitialAverage,f_sampleWeightInitialStdDev,f_crucibleWeightAverage,
   ...
   try:
      ...
      c = conn.cursor()
      # Insert initial data for the run
      t = (i_runID, i_crucibleID, f_sampleWeightInitialAverage,f_sampleWeightInitialStdDev,f_crucibleWeightAverage,f_crucibleWeightStdDev)
      logger.debug('insert into tblSample (i_runID,i_positionNumber,f_sampleWeightInitialAverage,f_sampleWeightInitialStdDev,f_crucibleWeightAverage,f_crucibleWeightStdDev) VALUES (%d,%d,%f,%f,%f,%f)' % (t))    #

      c.execute(
         'insert into tblSample (i_runID,i_positionNumber,f_sampleWeightInitialAverage,f_sampleWeightInitialStdDev,f_crucibleWeightAverage,f_crucibleWeightStdDev) VALUES (?,?,?,?,?,?)', t)    #
      conn.commit()
...
def updateSample(i_runID, i_crucibleID, v_description, d_datetime,
                    v_locationCode, i_preOrPost, i_loggingInterval, f_locationTemperature,
                    f_locationHumidity, v_status, i_preMeasurementTimeInterval, i_repetitions, f_crucibleWeightAverage,
                    f_crucibleWeightStdDev):
   ...
   try:
      ...
      t = (i_runID, i_crucibleID, v_description, d_datetime,
           ...
           f_crucibleWeightStdDev)
      logger.debug(
         'insert into tblSample (i_runID,i_positionNumber,v_description,d_datetime,v_locationCode,i_preOrPost,i_loggingInterval,f_locationTemperature,f_locationHumidity,v_status,i_preMeasurementTimeInterval,i_repetitions,f_crucibleWeightAverage,f_crucibleWeightStdDev) VALUES (%d,%d,%s,%s,%s,%d,%d,%f,%f,%s,%d,%d,%f,%f)' % (
         t))    #

      c.execute(
         'insert into tblSample (i_runID,i_positionNumber,v_description,d_datetime,v_locationCode,i_preOrPost,i_loggingInterval,f_locationTemperature,f_locationHumidity,v_status,i_preMeasurementTimeInterval,i_repetitions,f_crucibleWeightAverage,f_crucibleWeightStdDev) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)'
         , t)    #
      conn.commit()
      sampleID = c.lastrowid
      return (sampleID)
...
def getChamberCrucibleWeight(positionNumber):
   try:
      ...
      averageWeight = float(0.0)
      c.execute('select f_averageWeight from tblCrucibleXY where i_positionNumber=%d' % positionNumber)
      logger.debug('select f_averageWeight from tblCrucibleXY where i_positionNumber=%d' % positionNumber)
      data = c.fetchone()
      if (data is None):
         return False;
         returnValue = data["f_averageWeight"]
...
def updateRunPostFire(runID, setInitials, status, d_dateTimeFiring,
                      ...
                      durationOfPostMeasurements, repetitions, endOfFiring, startPosition, intervalsec,setHumidity,setTemperature):
   logger.debug("updateRunPostFire (   %d,%d,%d,%s,%s,%s,%d,%d,%d,%d,%s,%d )" %
                (setHumidity,setTemperature,runID, setInitials, status, d_dateTimeFiring, durationOfFiring, temperatureOfFiring,
                 postMeasurementTimeInterval,
                 durationOfPostMeasurements, endOfFiring, intervalsec))

   t = (setHumidity,setTemperature, intervalsec, status, d_dateTimeFiring, durationOfFiring, temperatureOfFiring, postMeasurementTimeInterval,
   ...
   # create a cursor to the database
      ...
      # Insert initial data for the run
      logger.debug(
         'update tblRun SET f_temperature=%d, f_humidity=%d, i_loggingInterval=%d,v_status=%s,d_dateTimeFiring=%s,i_durationOfFiring=%d,i_temperatureOfFiring=%d,i_postMeasurementTimeInterval=%d,i_durationOfPostMeasurements=%d,d_endOfFiring=%s WHERE i_runID=%d' % (
         t))
      c.execute(
         'update tblRun SET setHumidity=?,setTemperature=?,i_loggingInterval=?,v_status=?,d_dateTimeFiring=?,i_durationOfFiring=?,i_temperatureOfFiring=?,i_postMeasurementTimeInterval=?,i_durationOfPostMeasurements=?,d_endOfFiring=? WHERE i_runID=?'
         , t)
      # Save (commit) the changes
      conn.commit()
      # Now get the id for the run so we can update with other info as we ask...
