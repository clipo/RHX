...
def ask_for_excel_or_sqlite():
   ...
   choices = ["VTI-SA Data","RHX Environmental Chamber","Quit"]
...
def ask_for_weight_values():
   ...
   while 1:  # do forever, until we find acceptable values and break out
      if fieldValues is None:
         break
      errmsg = ""
      if int(fieldValues[0]) < int(fieldValues[2]):
         errmsg += ("The initial weight must be higher than the 105 degree weight ")
         # look for errors in the returned values
      if int(fieldValues[2]) < int(fieldValues[4]):
...
def ask_for_minmax_values(minx,maxx):
   ...
   while 1:  # do forever, until we find acceptable values and break out
      if fieldValues is None:
         break
      errmsg = ""
      if int(fieldValues[0]) > int(fieldValues[1]):
...
def graph_slope(sampleNumber,x,y,window,prefireWeightAverage,prefireWeightStdDev,postfireWeightAverage,postfireWeightStdDev):
...
def regression_figure(evenWeightPlot,xEvenArray, yEvenArray,yPercentEvenArray,weightSubPlot,xarray,yarray,xminval,xmaxval,ffilename,prefireWeightAverage,prefireWeightStdDev,postfireWeightAverage,postfireWeightStdDev,weightChange, percentWeightChange, runDate,sampleName):
   ...
   results += "Prefire (105) Absolute Weight (g): " + str(prefireWeightAverage)  + "+/-" + str(prefireWeightStdDev)+"\n\r"
   results += "Postfire (550) Absolute Weight (g): " + str(postfireWeightAverage) + "+/-" + str(postfireWeightStdDev)+"\n\r"
   results += "----------------------------------------------"+"\n\r"
...
def getRunInfo(runID):
   ...
   try:
      ...
      c = conn.cursor()
      logger.debug(
         "select v_operatorName, v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_endOfFiring,i_durationOfFiring from tblRun where i_runID = %d" % (
            runID))
      c.execute(
         'select v_operatorName, v_locationCode, i_numberOfSamples, v_description, f_locationTemperature, f_locationHumidity, d_endOfFiring,i_durationOfFiring from tblRun where i_runID=%s' % runID)
      row = c.fetchone()
      return row
   except sqlite.OperationalError, msg:
      logger.error("A SQL error occurred: %s" % msg)
...
## alternative is that its a sqlite file -- RHX chamber
   ...
   ffilename= easygui.fileopenbox(msg='SQLLite Filename', title='select file', filetypes=['*.sqlite'], default=directory)
   ...
   operator,sampleName, numberOfSamples, notes, temperatureLocation, humidityLocation, dateOfRun,i_durationOfFiring= getRunInfo(1)
   runDate = datetime.strptime(str(dateOfRun), '%Y-%m-%d %H:%M:%S')
...
while sampleNumber<=numberOfSamples:
   ...
   if ExcelOrRHX==1:
      ...
      try:
         c=conn.cursor()
         c.execute('select  f_weight, f_elapsedTimeQuarterPower, f_temperature, f_humidity from tblMeasurement where '
                   'i_positionNumber=%d and i_preOrPost=4  ' % sampleNumber)
         logger.debug('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                      'f_humidity from tblMeasurement where i_positionNumber=%d and i_preOrPost=4 ' % sampleNumber)
         for row in c.fetchall():
            count += 1
            if count==1:
               initialWeight=row[0]
            if count>0:
               ...
               temp.append(row[2])
               humid.append(row[3])
               ypercent.append((row[0]-initialWeight)/initialWeight)
      except sqlite.OperationalError,msg:
      ...
      overall_plot(ax1,x,y,sampleNumber)
      ...
      (slopeArray,interceptArray,r_valueArray,p_valueArray,std_errArray,dateArray)=graph_slope(
         sampleNumber,evenlySpacedXArray,evenlySpacedYPercentArray,window,
      ...
      results += regression_figure(evenPlotRegression,
         ...
         weightSubPlot,
         x,y,minval,maxval,
         ffilename,prefireWeightAverage,prefireWeightStdDev,postfireWeightAverage,postfireWeightStdDev,weightChange, percentWeightChange,runDate,sampleName)
