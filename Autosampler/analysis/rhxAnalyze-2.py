...
def ask_for_minmax_values(minx,maxx):
   ...
   while 1:  # do forever, until we find acceptable values and break out
      errmsg = ""
      if fieldValues == None:
         errmsg="You must enter something."
...
def correctForRH( yArray, humidArray, humiditySetting, rhCoefficent ):
...
temperatureLocation, humidityLocation=getTempAndHumidityForLocation()
...
while sampleNumber<=numberOfSamples:
   ...
   humidity, coefficient = ask_for_RHCoefficient(humidityLocation)
   ## define all of the plots in the figure
   ax1 = fig.add_subplot(5,2,1)
   weightSubPlot = fig.add_subplot(5,2,2)
   #filteredWeightPlot=fig.add_subplot(6,2,3)
   tempPlot = fig.add_subplot(5,2,3)               # filtered weight graph
   humidPlot = fig.add_subplot(5,2,4)              # the humidity graph
   slopeGraph = fig.add_subplot(5,2,5)
   dateGraph = fig.add_subplot(5,2,6)
   evenPlot = fig.add_subplot(5,2,7)
   evenPlotRegression = fig.add_subplot(5,2,8)
   correctedWeightPlot = fig.add_subplot(5,2,9)
   ...
   while (finish==0):
      ...
      correctedArray=[]
      correctedArray=correctForRH( y, humid, humidity, coefficient )
      #tempPlot.tick(labelsize=8)
      #humidPlot.tick(labelsize=8)
      correctedWeightPlot.scatter(x,correctedArray, marker='^', c='r')
      correctedWeightPlot.set_xlabel('Time (m^1/4)', fontsize=8)
      correctedWeightPlot.set_ylabel('RH Corrected Weight (g)', fontsize=8)
      correctedWeightPlot.grid(True)
      plt.ion()     # turns on interactive mode
      plt.show()    # now this should be non-blocking
      ...
      try:
         ...
         if (filterFlag=="Both"):
            ...
            c.execute('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                      'f_humidity from tblMeasurement where i_positionNumber=? and i_preOrPost=4 and f_humidity>? and'
                      ' f_humidity<? and f_temperature>? and f_temperature<? ', t)
            logger.debug('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                         'f_humidity from tblMeasurement where i_positionNumber=%d and i_preOrPost=4 and '
                         'f_humidity>%f and f_humidity<%f and f_temperature>%f and f_temperature<%f' % t)
            for row in c.fetchall():
               count += 1
               ##x.append(row['f_weight'])
               ##y.append(row['f_elapsedTimeQuarterPower'])
               ...
               c.execute('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                         'f_humidity from tblMeasurement where i_positionNumber=? and i_preOrPost=4 and '
                         'f_temperature>? and f_temperature<? ', t)
               logger.debug('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                            'f_humidity from tblMeasurement where i_positionNumber=%d and i_preOrPost=4 and '
                            'f_temperature>%f and f_temperature<%f' % t)
               for row in c.fetchall():
                  count += 1
                  ##x.append(row['f_weight'])
                  ##y.append(row['f_elapsedTimeQuarterPower'])
               ...
               c.execute('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                         'f_humidity from tblMeasurement where i_positionNumber=? and i_preOrPost=4 and f_humidity>? '
                         'and f_humidity<? ', t)
               logger.debug('select f_weight, f_elapsedTimeQuarterPower, f_temperature, '
                            'f_humidity from tblMeasurement where i_positionNumber=%d and i_preOrPost=4 and '
                            'f_humidity>%f and f_humidity<%f' % t)
               for row in c.fetchall():
                  count += 1
                  ##x.append(row['f_weight'])
                  ##y.append(row['f_elapsedTimeQuarterPower'])
      ...
      #correctedWeightPlot = fig.add_subplot(7,2,7)     # the filtered temperature graph
