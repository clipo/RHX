...
*   Date Generated: 06/30/10 15:43:00
*   Username:       Janus
*   System:         DHD5RV81
*
******************************************************************************/
...
char *pszTestName = "RandomGripperMoveTest";
char *pszTestPath = "c:\\packard\\janus\\bin\\Rehydroxylation\\";
...
/******************************************************************************
*
*   P01_Warn_User_to_Clear_Deck()
*
*   <Reserved for text entered into the procedure's comment page.>
*
******************************************************************************/

int P01_Warn_User_to_Clear_Deck()
{
    MP2_PROC_CONTEXT_DEF *pPC = &C01_Warn_User_to_Clear_Deck;
    int  nRet;              // Funct. return value
    int  nFileIndex = 0;    // Current File Index
    int  nLoopCount = 0;    // Number of unskipped procedure loops
    int  bDone;             // Last procedure loop flag
    ...
    for( pPC->nProcLoop = 0; pPC->nProcLoop <= pPC->nProcLimit; pPC->nProcLoop++ )
    {
        ...
        nRet = MSL_MessageDialog( 0,        // No dialog owner
                    "Warn User to Clear Deck",      // Dialog Title
                    "Remove EVERYTHING from the deck!

Remove any Plate Barcode Reader that may be present!
",      // Dialog Message
                    1,      // Icon: Information
                    1,      // Btns: OK
...
} //<End P01_Warn_User_to_Clear_Deck()>
...
int main(int argc, char **argv)
{
    ...
    nErr = P01_Warn_User_to_Clear_Deck();
    if( nErr ) return;

    nErr = P02_Test_Setup();
    if( nErr ) return;

    nErr = P03_Random_Move_Loop();
    if( nErr ) return;
