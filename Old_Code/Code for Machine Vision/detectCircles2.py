...
def detect_and_draw( img ):
    ...
    hsv_frame = cv.CreateImage(cv.GetSize(img), cv.IPL_DEPTH_8U, 3)
    thresholded = cv.CreateImage(cv.GetSize(img), cv.IPL_DEPTH_8U, 1)
    thresholded2 = cv.CreateImage(cv.GetSize(img), cv.IPL_DEPTH_8U, 1)
